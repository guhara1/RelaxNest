...
/* ----- 타이틀/디스크립션 헬퍼 -----
   - <title>: "지역 출장마사지 - 고유 설명 | RelaxNest" (지역+출장마사지로 시작, 페이지별 고유)
   - 디스크립션: 80자 이내, 페이지 고유 토큰(인접 동·역·focus)으로 중복 최소화
   - 카드 제목: 지역명만 사용(출장마사지 반복 제거 → 스팸/도어웨이 방지)            */
function pageTitle(place, descriptor) {
  return `${place} 출장마사지 - ${descriptor} | ${SITE.name}`;
}
function clamp80(s) {
  s = s.replace(/\s+/g, " ").trim();
  return [...s].slice(0, 80).join("");
}
function sampleNames(arr, n) {
  return (arr || []).slice(0, n).map((x) => x.name.replace(/(동|읍|면)$/, "")).join("·");
}
const UNIT = { seoul: "구", gyeonggi: "시군", incheon: "구군" };
...
function buildHome() {
  ...
  addPage({
    url: "/",
    title: pageTitle("서울·경기·인천", "수도권 홈타이 지역별 예약 안내"),
...
function buildRegions() {
  Object.values(REGIONS).forEach((r) => {
    ...
    addPage({
      url: `/${r.slug}/`,
      title: pageTitle(r.name, `${sampleNames(r.districts, 3)} 등 ${r.districts.length}개 ${UNIT[r.slug]} 예약 안내`),
      description: r.desc,
      body: hubBody,
      faq: hubFaq,
      reviews: true,
    ...
    r.life.forEach((l) => {
      ...
      addPage({
        url: `/${r.slug}/life/${l.slug}/`,
        title: pageTitle(l.name, `${r.name} ${l.focus} 생활권 예약`),
        description: l.desc,
        body: lBody,
        faq: lFaq,
        breadcrumb: lc
...
function subGuOf(r, d) {
  if (r.slug !== "gyeonggi") return null;
  return GYEONGGI_SUBGU.find((e) => e.city === d.slug) || null;
}
function subGuCards(r, d) {
  const entry = subGuOf(r, d);
  if (!entry) return [];
  return entry.gu.map((g) => ({
    url: `/gyeonggi/${d.slug}/${g.slug}/`,
    title: g.name,
    desc: `${g.focus} · ${sampleNames(g.dongs, 3)} 등`
  }));
}
function buildSubGu() {
  ...
  GYEONGGI_SUBGU.forEach((entry) => {
    ...
    if (!cityName) return;
    const cityBase = `/gyeonggi/${entry.city}/`;
    entry.gu.forEach((g) => {
      const base = `${cityBase}${g.slug}/`;
      const crumbs = [
        { name: "수도권 홈", url: "/" },
        { name: "경기", url: "/gyeonggi/" },
        { name: cityName, url: cityBase },
      ...
      const dongCards = dongs.map((d) => ({
        url: `${base}${d.slug}/`,
        title: d.name,
        desc: `${d.type} · 가까운 역 ${d.station}`
      }));
      const gFaq = [
        { q: `${cityName} ${g.name}도 방문 가능한가요?`, a: `${g.name} 전역과 인접 권역을 기준으로 정확한 방문 주소와 가까운 역(${g.stations})을 확인한 뒤 안내합니다.` },
        { q: `${g.name}에서 무엇을 먼저 확인해야 하나요?`, a: `${g.focus} 중심 권역으로 건물 출입 방식과 이용 장소(자택·호텔·오피스텔) 정책을 먼저 확인하면 좋습니다.` },
        { q: "불법·선정적 서비스도 가능한가요?", a: "불법·선정적 서비스는 제공하거나 안내하지 않습니다." }
      ];
      addPage({
        url: base,
        title: pageTitle(`${cityName} ${g.name}`, `${sampleNames(dongs, 3)} 행정동 예약 안내`),
        description: clamp80(`${cityName} ${g.name} ${sampleNames(dongs, 3)} 등 ${g.focus} 행정동 방문 예약 전 확인 안내`),
        body: `
        ${breadcrumb(crumbs)}
        ${hero({
          ...
          lede: g.char,
          cta: `<a class="btn btn-gold" href="/contact/">예약 문의</a><a class="btn btn-ghost" href="${cityBase}">${cityName} 전체</a>`,
        ...
        <section class="section section--tight">
          <div class="wrap">
            <div class="prose">
              ...
              <p>${g.char} 가까운 역은 ${g.stations}입니다. 소속 행정동은 ${dongs.map((d) => d.name).join(", ")}이며, 번호동은 대표 행정동으로 묶어 안내하고 출구별·노선별 페이지는 만들지 않습니다.</p>
              <h2>이용 장소별 확인사항</h2>
        ...
        ${dongCards.length ? linkCards(`${g.name} 행정동 바로가기`, "행정동별 안내", dongCards) : ""}
        ${pricing()}
        ${linkCards("관련 안내 · 이용 가이드", "내부 링크", [
          { url: cityBase, title: `${cityName} 전체 보기`, desc: `${cityName} 일반구·생활권 허브` },
          { url: "/gyeonggi/", title: "경기 전체 보기", desc: "경기 시군·생활권 허브" },
          { url: "/use/officetel/", title: "오피스텔 이용 안내", desc: "공동현관·관리 규정 확인" },
          { url: "/check/travel-fee/", title: "추가 이동비 기준", desc: "외곽 이동 사전 확인" }
        ])}
        ${policyNotice()}
        ${faqBlock(gFaq)}
        ${eeatBlock({
          who: `이 페이지는 ${SITE.author}가 작성하고 ${SITE.reviewer}가 검수합니다.`,
          how: `${cityName} ${g.name}의 공식 행정동(${dongs.map((d) => d.name).join("·")}), ${g.stations} 등 가까운 역 기준으로 구성했습니다.`,
      ...
      // 일반구 소속 행정동 페이지 (noindex, 사용자 탐색용)
      dongs.forEach((d) => buildDong(r, { name: g.name, char: g.char, focus: g.focus }, d, base, crumbs));
    });
  });
}
...
function buildStations(r) {
  ...
  addPage({
    url: base,
    title: pageTitle(`${r.name} 지하철역`, `${stations.slice(0, 3).map((s) => s.name).join("·")} 등 역세권 예약`),
    description: clamp80(`${r.name} ${stations.slice(0, 4).map((s) => s.name).join("·")} 등 주요 지하철역 역세권 방문 예약 안내`),
    body: `
    ${breadcrumb(crumbs)}
    ${hero({
  ...
  stations.forEach((s) => {
    ...
    addPage({
      url: `${base}${s.slug}/`,
      title: pageTitle(s.name, `${s.lines} 역세권 예약 안내`),
      description: clamp80(`${s.name} ${s.lines} ${s.focus} 역세권 방문형 관리 예약 전 확인 안내`),
      body: `
      ${breadcrumb(sc)}
      ${hero({
...
/* 이용 장소(롱테일) 내부링크: 동/구 성격(type/focus)에 맞는 가이드로 연결 */
function useLinkFor(type) {
  if (/오피스텔/.test(type)) return { url: "/use/officetel/", title: "오피스텔 이용 안내", desc: "공동현관·엘리베이터·관리 규정 확인" };
  if (/업무|행정/.test(type)) return { url: "/use/business-district/", title: "업무지구 이용 안내", desc: "방문자 등록·출입 가능 시간" };
  if (/상권|역세권/.test(type)) return { url: "/use/station-area/", title: "역세권 이용 안내", desc: "가까운 역·건물 위치 공유" };
  if (/공항|도서|항만/.test(type)) return { url: "/use/airport-island/", title: "공항·도서 지역 안내", desc: "사전 예약·이동 기준" };
  if (/숙소|호텔|관광/.test(type)) return { url: "/use/hotel/", title: "호텔·숙소 이용 안내", desc: "객실 출입 정책 확인" };
  return { url: "/use/home/", title: "자택 이용 안내", desc: "주소·공동현관 출입 확인" };
}

/* 행정동 페이지 (대표동·noindex) — 행정구/일반구 공용 */
function buildDong(r, parent, d, base, crumbs) {
  const gc = [...crumbs, { name: d.name, url: `${base}${d.slug}/` }];
  const gFaq = [
    { q: `${d.name}도 방문 가능한가요?`, a: `${d.name}(${parent.name}) 일대를 기준으로 정확한 방문 주소와 가까운 역(${d.station})을 확인한 뒤 안내합니다.` },
    { q: "건물 출입은 무엇을 확인하나요?", a: "공동현관·엘리베이터·관리 규정 등 건물 출입 방식과 방문 가능 시간을 확인합니다." }
  ];
  const gBody = `
  ${breadcrumb(gc)}
  ${hero({
    eyebrow: `${parent.name} · ${d.type}`,
    h1: `${d.name} 출장마사지 예약 안내`,
    lede: `${d.name}은 ${parent.name}의 ${d.type} 권역으로 가까운 역은 ${d.station}입니다.`,
    cta: `<a class="btn btn-gold" href="/contact/">예약 문의</a><a class="btn btn-ghost" href="${base}">${parent.name} 전체</a>`,
    alt: `${d.name} 방문형 관리 안내 이미지`
  })}
  <section class="section section--tight">
    <div class="wrap">
      <div class="prose">
        <p>${d.name}은 ${parent.name}에 속한 ${d.type} 성격의 권역입니다. ${parent.char}</p>
        <h2>${d.name} 이용 장소별 확인사항</h2>
        <p>가까운 역은 <strong>${d.station}</strong>입니다. 자택은 공동현관 출입 방식, 오피스텔은 관리 규정과 방문 가능 시간, 호텔·숙소는 객실 출입 정책을 먼저 확인하세요. 출구별·노선별 안내 대신 정확한 방문 주소와 가까운 출입구를 공유하면 이동이 원활합니다.</p>
        <h2>예약 전 체크리스트</h2>
        <ul>
          <li>정확한 방문 주소와 동·호수</li>
          <li>공동현관·건물 출입 방식</li>
  ...
  ${pricing()}
  ${linkCards("관련 안내 · 이용 가이드", "내부 링크", [
    { url: base, title: `${parent.name} 전체 보기`, desc: `${parent.name} 행정동 안내` },
    { url: `/${r.slug}/`, title: `${r.name} 전체 보기`, desc: `${r.name} 구·시군 허브` },
    useLinkFor(d.type),
    { url: "/check/building-access/", title: "건물 출입 방식 확인", desc: "공동현관·엘리베이터 안내" }
  ])}
  ${policyNotice()}
  ${faqBlock(gFaq)}
  ${eeatBlock()}`;
  ...
  addPage({
    url: `${base}${d.slug}/`,
    title: pageTitle(d.name, `${parent.name} ${d.station} 인근 예약`),
    description: clamp80(`${d.name}(${parent.name}) ${d.type} · ${d.station} 인근 방문형 관리 예약 전 확인 안내`),
    body: gBody,
    faq: gFaq,
    breadcrumb: gc,
    noindex: true
...
function buildDistrict(r, d) {
  ...
  const subgu = subGuOf(r, d);
  const dongs = subgu ? [] : (d.dongs || []); // 일반구가 있는 시는 일반구 아래에 행정동 배치
  const dongCards = dongs.map((g) => ({
    url: `${base}${g.slug}/`,
    title: g.name,
    desc: `${g.type} · 가까운 역 ${g.station}`
  }));
  const guCards = subgu ? subGuCards(r, d) : [];

  const dFaq = [
    { q: `${d.name} 어디까지 방문 가능한가요?`, a: `${d.name} 전역과 인접 권역을 기준으로 정확한 방문 주소와 가까운 역(${d.stations})을 확인한 뒤 안내합니다.` },
    { q: `${d.name}에서 무엇을 먼저 확인해야 하나요?`, a: `${d.focus} 중심 권역으로, 건물 출입 방식과 이용 장소(자택·호텔·오피스텔) 정책을 먼저 확인하면 좋습니다.` },
    { q: "불법·선정적 서비스도 가능한가요?", a: "불법·선정적 서비스는 제공하거나 안내하지 않습니다." }
  ];

  const childHeading = subgu ? "소속 일반구" : "대표 행정동";
  const childListHtml = subgu
    ? `<ul>${subgu.gu.map((g) => `<li><a href="${base}${g.slug}/">${g.name}</a> — ${g.focus}, ${g.stations}</li>`).join("")}</ul>`
    : (dongs.length
        ? `<ul>${dongs.map((g) => `<li><a href="${base}${g.slug}/">${g.name}</a> — ${g.type}, 가까운 역 ${g.station}</li>`).join("")}</ul>`
        : "<p>대표 행정동 페이지는 본문 품질이 확보되는 대로 순차 공개합니다.</p>");
  const childNames = subgu ? subgu.gu.map((g) => g.name) : dongs.map((g) => g.name);
  const h1desc = subgu ? "일반구·행정동별 예약 안내" : "행정동별 예약 안내";

  const dBody = `
  ${breadcrumb(crumbs)}
  ${hero({
    eyebrow: `${r.name} · ${d.focus}`,
    h1: `${d.name} 출장마사지 · ${h1desc}`,
    lede: d.char,
    cta: `<a class="btn btn-gold" href="/contact/">예약 문의</a><a class="btn btn-ghost" href="/${r.slug}/">${r.name} 전체</a>`,
    alt: `${d.name} 방문형 관리 안내 이미지`
  })}
  ...
  ${guCards.length ? linkCards(`${d.name} 일반구별 안내`, "일반구 바로가기", guCards) : ""}
  ...
  ${linkCards("관련 안내 · 이용 가이드", "내부 링크", [
    ...
    { url: `/${r.slug}/station/`, title: `${r.name} 역세권 안내`, desc: "주요 지하철역 모음" },
    useLinkFor(d.focus),
    { url: "/check/travel-fee/", title: "추가 이동비 기준", desc: "외곽 이동 사전 확인" }
  ])}
  ...
  ${eeatBlock({
    who: `이 페이지는 ${SITE.author}가 작성하고 ${SITE.reviewer}가 검수합니다.`,
    how: `${d.name}의 공식 ${subgu ? "일반구" : "행정동"}(${childNames.join("·")}), ${d.stations} 등 가까운 역 기준으로 구성했습니다.`,
    why: `${d.name}에서 방문형 서비스를 찾는 사용자가 ${subgu ? "일반구·행정동" : "행정동"}·이동 기준을 안전하게 확인할 수 있도록 작성했습니다.`
  })}`;

  addPage({
    url: base,
    title: pageTitle(d.name, subgu
      ? `${sampleNames(subgu.gu, 3)} 일반구 예약 안내`
      : `${sampleNames(dongs, 3) || d.focus} 행정동 예약 안내`),
    description: clamp80(subgu
      ? `${d.name} ${childNames.join("·")} 일반구 ${d.focus} 방문 예약 전 확인 안내`
      : `${d.name} ${sampleNames(dongs, 3)} 등 ${d.focus} 행정동 방문 예약 전 확인 안내`),
    body: dBody,
    faq: dFaq,
    breadcrumb: crumbs
  });

  // 행정동 페이지 (일반구 없는 경우만 — 일반구 시는 buildSubGu가 처리)
  dongs.forEach((g) => buildDong(r, d, g, base, crumbs));
}
